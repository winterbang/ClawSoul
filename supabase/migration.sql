...
-- 函数: 更新配置统计
CREATE OR REPLACE FUNCTION public.increment_config_stats()
RETURNS TRIGGER AS $$
BEGIN
    UPDATE profiles 
...
COMMENT ON FUNCTION public.increment_config_stats() IS '触发器函数：创建配置时自动增加用户的 total_configs 计数';

DROP TRIGGER IF EXISTS on_config_created ON configs;
CREATE TRIGGER on_config_created
    AFTER INSERT ON configs
    FOR EACH ROW EXECUTE FUNCTION public.increment_config_stats();
...
COMMENT ON FUNCTION public.record_download(UUID, UUID, TEXT, TEXT) IS '记录下载操作，同时更新配置和用户的下载统计。参数: p_config_id(配置ID), p_user_id(用户ID,可为空), p_format(格式), p_source(来源)';
...
COMMENT ON FUNCTION public.increment_fork_count(UUID) IS '增加指定配置的 fork_count 计数';
...
-- configs 表策略
DROP POLICY IF EXISTS "用户可查看自己的配置和公开配置" ON configs;
CREATE POLICY "用户可查看自己的配置和公开配置"
    ON configs FOR SELECT
    USING (auth.uid() = user_id OR is_public = TRUE);

DROP POLICY IF EXISTS "用户可创建自己的配置" ON configs;
CREATE POLICY "用户可创建自己的配置"
    ON configs FOR INSERT
    WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "用户可更新自己的配置" ON configs;
CREATE POLICY "用户可更新自己的配置"
    ON configs FOR UPDATE
    USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "用户可删除自己的配置" ON configs;
...
-- downloads 表策略
DROP POLICY IF EXISTS "用户可查看自己的下载记录" ON downloads;
...
-- activity_logs 表策略
DROP POLICY IF EXISTS "用户可查看自己的活动日志" ON activity_logs;
...
-- config_versions 表策略
DROP POLICY IF EXISTS "用户可查看自己的配置版本" ON config_versions;
CREATE POLICY "用户可查看自己的配置版本"
    ON config_versions FOR SELECT
    USING (auth.uid() IN (
        SELECT user_id FROM configs WHERE id = config_versions.config_id
    ));

COMMENT ON POLICY "用户可查看自己的配置版本" ON config_versions IS '用户只能查看自己配置的版本历史';
...
-- 完成！
-- =====================================================
SELECT 'Database setup complete! All tables and comments added.' as status;
